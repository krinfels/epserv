 ...
 *   tell_tracker: inform the tracker server about a signal/dead proc
 */
 ...
 *  find_tracker
 *-----------------------------------------------------------------------------*/
endpoint_t find_tracker() {
	register struct mproc *rmp;

	for(rmp = &mproc[0]; rmp < &mproc[NR_PROCS]; rmp++)
		if((rmp->mp_flags & IN_USE) && !strcmp("tracker", rmp->mp_name))
 ...
 *				tell_tracker					     *
 *===========================================================================*/
void tell_tracker(endpoint_t ep, pid_t p, int type) {
	message msg;
	endpoint_t tracker_proc_nr = find_tracker();

	msg.m_type = type;
	msg.m1_i1 = ep;
	msg.m1_i2 = p;

	if(tracker_proc_nr != -1)
		asynsend3(tracker_proc_nr, &msg, AMF_NOREPLY); 
}
